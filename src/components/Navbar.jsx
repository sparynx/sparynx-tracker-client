...
const Navbar = () => {
  ...
  const getInitials = (email) => {
    if (!email) return "";
    return email.split("@")[0].split(/[.\-_]/).map((word) => word.charAt(0).toUpperCase()).join("");
  };
    ...
    <>
      ...
      <nav className="flex items-center justify-between bg-gray-800 text-white p-4 fixed w-full top-0 z-50">
        ...
        <div className="hidden lg:flex flex-grow justify-center">
          <ul className="flex flex-row gap-8 whitespace-nowrap">
            {navbarItems.map((item) => (
              <li key={item.name}>
                <Link to={item.href} className="text-white hover:underline hover:text-gray-400 transition duration-300 ease-in-out">
                  {item.name}
                </Link>
              </li>
            ))}
        ...
        <div className="hidden lg:flex items-center space-x-4">
          ...
          <div className="flex items-center bg-gray-700 rounded-lg p-1">
            <IoMdSearch size={20} />
            <input type="text" placeholder="Search..." className="px-2 py-1 bg-gray-700 text-white focus:outline-none rounded" />
          </div>
              ...
              <>
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-gray-700 shadow-lg rounded-md z-40">
                    <ul className="py-2">
                      <li>
                        <Link to="/profile" className="block px-4 py-2 text-sm text-white hover:bg-gray-600" onClick={() => setIsDropdownOpen(false)}>
                          Profile
                        </Link>
                      </li>
                      <li>
                        <button className="block px-4 py-2 text-sm text-white hover:bg-gray-600 w-full text-left" onClick={handleLogOut}>
                          Logout
                        </button>
                      </li>
                    </ul>
      ...
      {isMenuOpen && (
        <div className="lg:hidden fixed top-0 left-0 w-64 h-full bg-gray-800 p-6 z-50 flex flex-col shadow-lg">
          ...
          {currentUser && (
            <button className="mt-4 w-full text-left text-white hover:underline hover:text-gray-400" onClick={handleLogOut}>
              Logout
            </button>
          )}
        </div>
